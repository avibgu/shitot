...
public class BlindSearch {
    ...
    public Pair search (Horner hor,double a,double h){
     ...
     public Pair search (Function func,double a,double h){
        if (count ==0){ // Reached maximum iterations.
            System.out.println("Bad Values!!!");
            return new Pair(a,a+h);
        }
        ...
        double b = a + h;
        double c = a - h;
        double f_a = func.Calculate(a); // calulates f(a) using horner algo.
        double f_b = func.Calculate(b); // calulates f(b) using horner algo.

       if(f_a*f_b >= 0){
           double f_c = func.Calculate(c); // calulates f(c) using horner algo.
           if (f_c * f_a < 0){
               b = a;
               a = c;
               return new Pair(a,b);
              ...
              count--; //decreasing the iteration counter.
              return search(func,a,h); // Recursive call with the new parameters.
            }
       }
       else return new Pair(a,b);
  }
